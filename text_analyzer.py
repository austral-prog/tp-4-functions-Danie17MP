...
def total_letters(text):
    ...
    """
    letters = count_vowels(text) + count_consonants(text)
...
def vowel_percentage(text):
    ...
    if total_letters(text) > 0:
        ptg = (count_vowels(text) / total_letters(text)) * 100
    return round(ptg, 1)  # Remove this line and implement
...
def analyze_text(text):
    ...
    """
    analysis = f"V:{count_vowels(text)} C:{count_consonants(text)} T:{total_letters(text)} P:{vowel_percentage(text)}%"
    return analysis  # Remove this line and implement
